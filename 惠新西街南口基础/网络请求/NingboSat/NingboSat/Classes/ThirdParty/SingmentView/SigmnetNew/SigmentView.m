...
//  HZSigmentView
//
//  Created by 王会洲 on 16/11/23.
//  Copyright © 2016年 王会洲. All rights reserved.
//

#import "SigmentView.h"

@interface SigmentView()
...
-(instancetype)initWithOrgin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        self.titleColorSelect = DDMColor(75, 196, 251);
        self.bottomLineColor = DDMColor(214, 214, 214);
        self.titleLineColor = DDMColor(75, 196, 251);
...
-(UIView *)bottomLine {
    if (!_bottomLine) {
        _bottomLine = [[UIView alloc] initWithFrame:CGRectMake(((DDMWIDTH * 0.5) - BottomLineWidth) * 0.5, self.menuHeight - BottomLineHeight, BottomLineWidth, BottomLineHeight)];
...
-(void)setTitleArry:(NSArray *)titleArry {
    ...
    [UIView animateWithDuration:0.15 animations:^{
        self.bottomLine.frame = CGRectMake(((DDMWIDTH * 0.5) - BottomLineWidth) * 0.5, self.menuHeight - BottomLineHeight, BottomLineWidth, BottomLineHeight);
    }];
...
- (void)setIndex:(NSInteger)index {
    if (index >= 0 && index < self.btnArrys.count) {
        [self btnTitleClick:self.btnArrys[index]];
    }
}
...
-(void)btnTitleClick:(UIButton *)sender {
    ...
    [UIView animateWithDuration:0.15 animations:^{
        ...
        self.bottomLine.frame=CGRectMake(sender.frame.origin.x + ((DDMWIDTH * 0.5) - BottomLineWidth) * 0.5, self.frame.size.height - BottomLineHeight, BottomLineWidth, BottomLineHeight);
    }];
...
- (void)scrollMenuViewSelectedoffsetX:(NSInteger)selectIndex withOffsetType:(BOOL)types {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        [self.BackScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
        if (types) {
            self.bottomLine.frame=CGRectMake( DDMWIDTH * 0.5 * selectIndex + ((DDMWIDTH * 0.5) - BottomLineWidth) * 0.5, self.frame.size.height - BottomLineHeight, BottomLineWidth, BottomLineHeight);
