...
-(instancetype)initWithOrgin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        self.titleFont = DDMFONTLight_BY_SCREEN(14); //[UIFont systemFontOfSize:15];
        self.btnArrys = [[NSMutableArray alloc] initWithCapacity:0];
        self.titleColorNormal = DDMColor(80, 80, 80);
        self.titleColorSelect = DDMColor(88, 207, 252);
        self.bottomLineColor = DDMColor(255, 255, 255);
        self.titleLineColor = DDMColor(75, 196, 251); // 底部横线位置
        
        self.BackScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, DDMWIDTH, self.menuHeight)];
        self.BackScrollView.backgroundColor = DDMColor(255, 255, 255);
...
//    }
    ...
    /**修改上下滑动bug*/
    self.BackScrollView.contentSize = CGSizeMake(self.btnWidth * _titleArry.count, -10);
...
-(void)btnTitleClick:(UIButton *)sender {
    ...
    if (sender.tag == self.defaultIndex) {
        ...
        self.titleBtn.titleLabel.font =  DDMFONTMedium_BY_SCREEN(16);
    }
    //计算偏移量
