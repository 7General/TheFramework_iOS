...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSArray *)imgArr TitleArray:(NSArray *)titArr {
	if (self = [super initWithFrame:rect]) {
        ...
        for (int i=0; i<pageCount; i++) {
            ...
            if ([imgURL hasPrefix:@"http://"]) {
                //网络图片 请使用ego异步图片库
                [imgView sd_setImageWithURL:[NSURL URLWithString:imgURL] placeholderImage:[UIImage imageNamed:@"bannerPlaceholder"]];
            }
            else {
        ...
        noteView.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:0.3];
        [self addSubview:noteView];
        
        float pageControlWidth = (pageCount-2)*10.0f+40.f;
        float pagecontrolHeight = 20.0f;
        pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake((self.frame.size.width-pageControlWidth), (CGRectGetHeight(noteView.bounds) - pagecontrolHeight) / 2, pageControlWidth, pagecontrolHeight)];
        ...
        noteTitle=[[UILabel alloc] initWithFrame:CGRectMake(5, self.bounds.size.height-27, self.frame.size.width-pageControlWidth-15, 20)];
    ...
    [NSTimer scheduledTimerWithTimeInterval:3.0f target:self selector:@selector(moveBanner) userInfo:nil repeats:YES];
...
-(void)moveBanner{
    ...
    [UIView animateWithDuration:0.5f animations:^{
        scrollView.contentOffset = CGPointMake(currentPageIndex*CGRectGetWidth(scrollView.bounds), 0);
    }];
