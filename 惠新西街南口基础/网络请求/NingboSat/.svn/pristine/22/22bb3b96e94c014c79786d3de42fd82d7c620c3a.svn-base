...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(NotifiResetButton) name:@"PMPOST" object:nil];
}
...
-(void)setAMDataArry:(NSMutableArray *)AMDataArry {
    _AMDataArry = AMDataArry;
    for (UIView * view in self.centerView.subviews) {
        [view removeFromSuperview];
    }
    
    int appW = (SCREEN_WIDTH - 20) / 3;
    [self rankWithTotalColumns:3 andWithAppW:appW andWithAppH:70 andWithPadding:10 targetView:self.centerView rowData:self.AMDataArry];
}
...
-(void)leftBookClick:(UIButton *)sender {
    ...
    /**代理传值*/
    if (self.delegate && [self.delegate respondsToSelector:@selector(didselectObjModelAM:)]) {
        SelectBookTimeModel * model = self.AMDataArry[sender.tag - 1000];
        [self.delegate didselectObjModelAM:model];
    }
    /**左边按钮发送通知清除上一个选中状态*/
    [[NSNotificationCenter defaultCenter] postNotificationName:@"AMPOST" object:nil];
}
-(void)rightBookClick:(UIButton *)sender {
    ...
    /**代理传值*/
    if (self.delegate && [self.delegate respondsToSelector:@selector(didselectObjModelAM:)]) {
        SelectBookTimeModel * model = self.AMDataArry[sender.tag - 3000];
        [self.delegate didselectObjModelAM:model];
    }
    /**左边按钮发送通知清除上一个选中状态*/
    [[NSNotificationCenter defaultCenter] postNotificationName:@"AMPOST" object:nil];
