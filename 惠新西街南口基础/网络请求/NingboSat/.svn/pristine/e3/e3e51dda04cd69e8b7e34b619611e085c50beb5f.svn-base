...
@interface SigmentScrollView()<SigmentViewDelegate>
/**操作console*/
@property (nonatomic, strong) UIScrollView * BackScrollView;
@property (nonatomic, strong) SigmentView * sigmentView;
...
@implementation SigmentScrollView
-(SigmentView *)sigmentView {
    if (!_sigmentView) {
        _sigmentView = [[SigmentView alloc] initWithOrgin:CGPointMake(0, 0) andHeight:44];
    }
    return _sigmentView;
}
...
- (void)updateUIForKeypath:(NSString *)keyPath {
    if([keyPath isEqualToString:@"SelectDefaultIndex"]) {
       [self.sigmentView setIndex:self.SelectDefaultIndex];
    }
}
...
-(void)segment:(SigmentView *)sengment didSelectColumnIndex:(NSInteger)index {
